...
class LLMMitigationAverageMMLU(LLMMitigationMetric):
    ...
    def compute(self, predictions_dict: Dict[str, Dict[str, float]], model_targets_dict: Dict[str, Dict[str, float]],
        ...
        for model_name in model_names_to_process:
            mmlu_vals.append(predictions_dict[model_name]['mmlu'])

        return {'result': np.average(mmlu_vals).item(), 'files': None}

    def compare(self, computed, baseline):
        return computed > baseline


class LLMMitigationAveragePoisonedMMLU(LLMMitigationAverageMMLU):
    def __init__(self):
        super().__init__(name='Avg MMLU (poisoned model)', clean_only=False, poisoned_only=True)
...
class LLMMitigationAverageCleanMMLU(LLMMitigationAverageMMLU):
    def __init__(self):
        super().__init__('Avg MMLU (clean model)', clean_only=True, poisoned_only=False)


class LLMMitigationAverageAllMMLU(LLMMitigationAverageMMLU):
    def __init__(self):
        super().__init__('Avg MMLU (all)', clean_only=True, poisoned_only=True)


class LLMMitigationFidelityMetric(LLMMitigationMetric):
    def __init__(self, write_html: bool = True, share_with_actor: bool = False, store_result: bool = True, share_with_external: bool = False):
        super().__init__(write_html, share_with_actor, store_result, share_with_external)

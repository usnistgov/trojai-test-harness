...
class Submission(object):
    ...
    def execute(self, actor: Actor, trojai_config: TrojaiConfig, execution_epoch: int) -> None:
        ...
        cmd_str_list = ['sbatch', '--partition', control_slurm_queue, '--parsable', '--priority', str(self.slurm_priority), '--nodes', '1', '--ntasks-per-node', '1', '--cpus-per-task', '1', ':', '--partition', self.slurm_queue_name, '--priority', str(self.slurm_priority), '--nodes', '1', '--ntasks-per-node', '1', '--cpus-per-task', str(cpus_per_task), '--exclusive', '-J', self.active_slurm_job_name, '--parsable', '-o', slurm_output_filepath, slurm_script_filepath, actor.name, actor.email, submission_filepath, self.execution_results_dirpath, trojai_config_filepath, self.leaderboard_name, self.data_split_name, test_harness_dirpath, python_executable, task_executor_script_filepath]

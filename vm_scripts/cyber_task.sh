...
while [[ $# -gt 0 ]]; do
  ...
  --scale-params-filepath)
    shift
    SCALE_PARAMS_FILEPATH="$1" ;;
  *)
    EXTRA_ARGS+=("$1") ;;
  esac
...
if [ -z "${SOURCE_DATA_DIR-}" ]; then
  singularity run --contain --bind "$ACTIVE_DIR" --bind "$SCRATCH_DIR" \
    ...
    --schema_filepath "$METAPARAMETERS_SCHEMA_FILE" --learned_parameters_dirpath "$LEARNED_PARAMETERS_DIR" \
     --scale-params-filepath "$SCALE_PARAMS_FILEPATH" >> "${RESULT_DIR}/${CONTAINER_NAME}.out" 2>&1
else
  singularity run --contain --bind "$ACTIVE_DIR" --bind "$SCRATCH_DIR" \
    ...
    --round_training_dataset_dirpath "$ROUND_TRAINING_DATASET_DIR" --metaparameters_filepath "$METAPARAMETERS_FILE" \
    --schema_filepath "$METAPARAMETERS_SCHEMA_FILE" --learned_parameters_dirpath "$LEARNED_PARAMETERS_DIR" \
    --source_dataset_dirpath "$SOURCE_DATA_DIR" --scale-params-filepath "$SCALE_PARAMS_FILEPATH" >> "${RESULT_DIR}/${CONTAINER_NAME}.out" 2>&1
fi